...
ANSIBLE_GALAXY_COLLECTIONS=${ANSIBLE_GALAXY_COLLECTIONS:=$ANSIBLE_PROJECT_FOLDER/.collections}
...
if [ -r "$ANSIBLE_GALAXY_ROLEFILE" ]; then
  ...
  export ANSIBLE_COLLECTIONS_PATH=$ANSIBLE_GALAXY_COLLECTIONS
  if [ ! -d $ANSIBLE_GALAXY_ROLES ]; then
    mkdir -p $ANSIBLE_GALAXY_ROLES
    mkdir -p $ANSIBLE_GALAXY_COLLECTIONS
  fi
    OLD_ANSIBLE_ROLES_PATH=$ANSIBLE_ROLES_PATH
    export ANSIBLE_ROLES_PATH=$ANSIBLE_GALAXY_ROLES
    ansible-galaxy install --role-file=$ANSIBLE_GALAXY_ROLEFILE
    ANSIBLE_ROLES_PATH=$OLD_ANSIBLE_ROLES_PATH
